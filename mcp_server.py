...
CHROMA_PERSIST_DIR = "rkm-agent"

# Initialize the FastMCP server with a descriptive name
mcp = FastMCP("RAGAssistant")

import os
os.environ["USER_AGENT"] = "MyCoolBot/1.0 (contact@example.com)"

@mcp.tool()
async def ingest_document(url: str, ctx: Context) -> str:
    """
    Loads a webpage from a URL, splits it into chunks, generates
    embeddings using OllamaEmbeddings, and stores them in a persistent
    ...
    Args:
        url: The absolute or relative path to the webpage.
                   For example: "https://belurmath.org/".
    ...
    try:
        ...
        # NOTE: Use a dedicated embedding model, not a chat model.
        ...
        loader = RecursiveUrlLoader(
            url=url,
            max_depth=2,  # Adjust how deep into the link structure to crawl
            extractor=lambda x: BeautifulSoup(x, "html.parser").text,  # Extracts raw text
            # Optional: Prevent crawling outside the main documentation path
        ...
        if not chunks:
            await ctx.error("Error: Could not extract any text chunks from the document. The file might be empty.")
        ...
        file_name = url
        await ctx.info(f"Successfully ingested the document from '{file_name}' into the vector store.")
...
@mcp.tool()
async def query_rag_store(query: str, ctx: Context) -> str:
    """
    ...
    try:
        ...
        await ctx.info(f"Created context - {context}")
        return context
